...
window.addEventListener('settingsReady', function(event) { //after domcontentloaded
  ...
  const updateLive = (message, color, showRelativeTime) => {
    ...
    if (livetext) {
      ...
      console.log('no livetext to set value to', message)
    }
  }
  let lastPostIds = {};
  let liveTimeout;
  ...
  const fetchNewPosts = new Promise(function(resolve, reject) {
    console.log('fetching posts from api', jsonPath);
    updateLive('Fetching posts...', 'yellow');
    let json;
    let newPosts = [];
    try {
      fetch(jsonPath).then(res => res.text()).then(function(text) {
        let json
        try {
          json = JSON.parse(text)
        } catch(e) {
          // don't expect this to work for now
          //console.error('json parse failure', text, e)
          //return reject(e)
          return resolve(0)
        }
  ...
  forceUpdate = () => {
    ...
    fetchNewPosts().then(function(posts) {
      if (posts > 0) {
        interval = 5000;
      } else {
