...
const updateFiltersTable = () => {
  var children = []
  for(var i in filtersTable.children) {
    if (filtersTable.children.hasOwnProperty(i)) {
      children.push(filtersTable.children[i])
    }
  }
  children.slice(3).forEach(row => row.remove());
